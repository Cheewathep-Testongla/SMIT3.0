...
def TestSpellCheck(Data):
    ...
    ListUnwantedText = ['[a-z]+-[0-9a-z]+-[a-z0-9]+',               #a-a0-a0     
    ...
    for sentence in Tokenize_Input:
        ...
        for i in range(len(ListTokenize)):    
            ...
            if(len(re.findall('[a-z]', ListTokenize[i])) == 1):
                continue
            elif(len(re.findall(' [a-z][a-z][a-z] | [a-z][a-z] ', ListTokenize[i])) > 1):
                TempResult.append(ListTokenize[i])
            elif(ListTokenize[i] in DictTokenize):
                IndexRepeatDictTokenize = []
                # Get all duplicate value in DictTokenize that match with ListTokenize[i]
                for idx, val in enumerate(DictTokenize):
                    if ListTokenize[i] == val and idx not in IndexRepeatDictTokenize:
...
# test = 'ตั่งนังร๊าน ยกของง , weding' 
...
# print(GoogleTranslator(source='auto', target='en').translate('ตั่งนั่งร้าน ยกของ welding'))
# เพิ่ม Function บนหน้าเว็บให้ผู้ใช้งานสามารถเลือกได้ว่าต้องการเลือก list ไหนมาแสดงบ้าง และ สามารถแก้ไขข้อมูลใน list นั้น ๆ ได้ : อาจจะไม่ทำ
# แก้ไข Dict ให้เพิ่มคำที่ถูก เข้าไปใน Column words : อาจจะไม่ทำ
# ถ้า List Tokenize มีแค่ 1 value จะรวมกับ List ก่อนหน้า
# ตอนส่ง Corrected Input กลับไปยัง Power App จะส่ง Input ที่ถูกแก้คำผิดแล้ว กับ keyword ที่ถูกตัดกลับไป